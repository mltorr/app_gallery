...
utils.setup_page("Invoice Query")
...
def main():
    ...
    if st.sidebar.button("Sample Prompt 1"):
        st.session_state.input_prompt = "Please parse the invoice number, invoice date, purchase order no. and the details table and put them on a single table (values on the table can be repeated per row)"
        st.experimental_rerun()
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            ...
            if file_extension == "pdf":
                ...
                all_filenames.extend([uploaded_file.name] * len(images_data))
            else:
    ...
    if st.button("Send"):
        ...
        with st.spinner("Start processing..."):
            for img_data, filename in zip(all_images, all_filenames):
                ...
                if not df.empty:
                    ...
                    df = remove_invalid_rows(df)  # Remove invalid rows
                    table_structure = tuple(df.columns)
                    if table_structure not in all_dataframes:
                        all_dataframes[table_structure] = df
                    else:
                        all_dataframes[table_structure] = pd.concat([all_dataframes[table_structure], df], ignore_index=True)

        if all_dataframes:
            for structure, dataframe in all_dataframes.items():
                # st.subheader(f"Combined Table for Structure: {structure}")
